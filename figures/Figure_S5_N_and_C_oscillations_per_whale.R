...
# 2. Import data
fw.data_d13cor <- read_excel("All_merged_time_calibrated_2013_to_2022_Suess_cor.xlsx")
...
its_per_whale <- isop + theme_article(base_size = 20) + scale_fill_manual(values = legend, labels = c(expression(paste(delta^{13}, "C (\u2030)")), expression(paste(delta^{15}, "N (\u2030)")))) + theme(legend.title = element_blank(), legend.position = c(0.95,0.1))
