#--------------------------------------------------------------------------------
#Sliding window analysis with the time-calibrated stable isotope data (carbon)
#--------------------------------------------------------------------------------
...
anova(wo_lenght_model, lenght_model, test="Chisq") # Decision: Evidence lenght improve the model.
...
Bp_dC_clim_m_rel <- 
  climwin::slidingwin(baseline = lme4::lmer(d13c.cor ~ 1 + poly(Bp_isotopic_data_use$julian_day,2) + (1| Bp_isotopic_data_use$whale_id), data=Bp_isotopic_data_use),  #this is the base model to which the program adds the climate variables
...
Bp_dC_clim_m_rel$combos #check all of the sliding windows in the analyses

#AIC difference < 2
a<-head(Bp_dC_clim_m_rel[[1]]$Dataset, 20) ## Lin_func Mean NAO, close window-open window (24-6) (beta coefficient= 0.16)
a <- a %>% filter(deltaAICc < (a$deltaAICc[1])+2)
nrow_a <- nrow(a)
a$Index <- c(rep("NAO", nrow_a))
b<-head(Bp_dC_clim_m_rel[[2]]$Dataset, 20) ## Lin_func Mean AMO, close window-open window (10-9) (beta coefficient= -0.54)
b <- b %>% filter(deltaAICc < (b$deltaAICc[1])+2)
nrow_b <- nrow(b)
b$Index <- c(rep("AMO", nrow_b))
c<-head(Bp_dC_clim_m_rel[[3]]$Dataset, 20) ## Lin_func Mean AMOC, close window-open window (33-23) (beta coefficient= -0.03)
...
Bp_isotopic_data_use$mean_AMOC_linear_start_m <- ymd(Bp_isotopic_data_use$measured)%m-% months(33)
Bp_isotopic_data_use$mean_AMOC_linear_end_m <- ymd(Bp_isotopic_data_use$measured)%m-% months(23)
...
for (i in 1:1381) { # loop from one to number of samples samples 
  # Average NAO index
  mean_NAO_dC_list[[i]] <- data_climate %>% 
  ...
  # Average AMO index
  mean_AMO_dC_list[[i]] <- data_climate %>% 
  ...
  mean_AMOC_dC_data <- bind_rows(mean_AMOC_dC_list)
...
Bp_dC_final <- dplyr::bind_cols(Bp_isotopic_data_new, Bp_dC_windows)


### Save data

write.csv(Bp_dC_final, "/Users/marcruizisagales/Documents/GitHub/Climate-baleen-plate-isotopes/data/Bp_isotopic_climate_summarised_final_carbon_mean_seasonality_for_Github.csv")
